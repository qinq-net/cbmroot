...
ClassImp(CbmMuchMcbm)

  // -----   Default constructor   -------------------------------------------
CbmMuchMcbm::CbmMuchMcbm() 
: FairDetector(),
  fTrackID(-1),
  fVolumeID(-1),
  fPosIn(),
...
CbmMuchMcbm::CbmMuchMcbm(const char* name, Bool_t active)
...
Bool_t CbmMuchMcbm::ProcessHits(FairVolume* vol) {
...
// -------------------------------------------------------------------------
void CbmMuchMcbm::BeginEvent() {
}
// -------------------------------------------------------------------------


// -------------------------------------------------------------------------
void CbmMuchMcbm::EndOfEvent() {
...
// -------------------------------------------------------------------------
TClonesArray* CbmMuchMcbm::GetCollection(Int_t iColl) const {
...
// -------------------------------------------------------------------------
void CbmMuchMcbm::Print(Option_t*) const {
...
CbmMuchPoint* CbmMuchMcbm::AddHit(Int_t trackID, Int_t detID, TVector3 posIn,
...
//  TGeoBBox* shModuleFull = new TGeoBBox("shModule", mDx, mDy, mDz);
  ...
  for (Int_t iStation=0;iStation<fGeoScheme->GetNStations();iStation++){
    ...
    if (nLayers<=0) {
      Warning("CbmMuchMcbm","Station layers are not defined for station %02i",iStation);
      continue;
    }
...
//            TGeoBBox* shActiveBox = new TGeoBBox(moduleBoxName, size[0] / 2., size[1] / 2., size[2] / 2.);
          ...
          if (module->GetDetectorType()==2) {
            Fatal("CbmMuchMcbm","Station %02i - detailed module design not implemented for straws",iStation);
          }
...
Int_t CbmMuchMcbm::GetDetId(FairVolume* vol) {
...
// -------------------------------------------------------------------------
TGeoMedium* CbmMuchMcbm::CreateMedium(const char* matName){
  FairGeoLoader*    geoLoad   = FairGeoLoader::Instance();
  FairGeoBuilder*   geobuild  = geoLoad->getGeoBuilder();
  FairGeoInterface* geoFace   = geoLoad->getGeoInterface();
  FairGeoMedia*     media     = geoFace->getMedia();
...
void CbmMuchMcbm::ExpandMuchNodes(TGeoNode* fN)
{
/*
