  ...
  
{
  ...
  if (fRecoLevel==-1 || fRecoLevel>10) model = new InverseSlope(fMass, fPDGID, false, -ycm, ycm, ycm, 1.);
  else model = new InverseSlope(fMass, fPDGID, true, -ycm, ycm, ycm, 1.);
...
void CbmInverseSlope::AddRapidityInterval(double ymin, double ymax)
{
	...
	if (fRecoLevel==-1 || fRecoLevel>10) modelsY.push_back(new InverseSlope(fMass, fPDGID, false, ymin, ymax, ycm, 1.));
	else modelsY.push_back(new InverseSlope(fMass, fPDGID, true, ymin, ymax, ycm, 1.));
}
...
void CbmInverseSlope::Finish(){
  ...
  for(int n = 1; n < histodndymodel->GetNbinsX(); n++) {
	  histodndymodel->SetBinContent(n, RapidityFitNamespace::TwoGauss(histodndymodel->GetXaxis()->GetBinCenter(n), 
			params.A.value, params.sig.value, params.yav.value));
	  //model->fmt(histo1D[IndexModelMt]->GetXaxis()->GetBinCenter(n), T));
  }
  
  std::cout << "<" << name << "> = " << params.A.value << "  " << "Error = " << params.A.error << "\n";
}

void CbmInverseSlope::CalculateAveragesInEvent(int RecoLevel, bool UpdateGlobal) {
  if (RecoLevel==-1) {
