...
void CbmStsDigitizeQa::CreateDigiHistograms(){
	...
	fHM->Create2<TH2F>(Form("h_DigisPerChip_Station%i",stationId), Form("Digis per Chip, Station %i;x, cm;y, cm", stationId), 400, -50, 50, 200, -50, 50);
	fHM->Create2<TH2F>(Form("h_PointsMap_Station%i",stationId), Form("Points Map, Station %i;x, cm;y, cm", stationId), 100, -50, 50, 100, -50, 50);
	fHM->Create2<TH2F>(Form("h_MeanAngleMap_Station%i",stationId), Form("Mean Angle Map, Station %i;x, cm;y, cm", stationId), 50, -50, 50, 50, -50, 50);
	fHM->Create2<TH2F>(Form("h_RMSAngleMap_Station%i",stationId), Form("RMS Angle Map, Station %i;x, cm;y, cm", stationId), 50, -50, 50, 50, -50, 50);
    }
	...
	//	cout << "global (0,1) = " << global[0] << ", " << global[1] << endl;
	fHM->Create1<TH1F>(Form("h_ParticleAngles_%s", modu->GetName()), Form("Particle Angles (%.0f cm, %.0f cm);Angle, deg;Entries", global[0], global[1]), 90, 0., 90.);
    }
}

void CbmStsDigitizeQa::ProcessDigisAndPoints(const TClonesArray* digis, const TClonesArray * points){
    // std::cout << "we are in ProcessDigisAndPoints" << std::endl;
	...
	if (nOfChannelsM != 2048) cout << ", modu->GetNofChannels = " << nOfChannelsM << endl;
