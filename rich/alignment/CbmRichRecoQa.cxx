...
void CbmRichRecoQa::InitHistograms()
{
    ...
    for (Int_t i = 0; i < 4; i++){
		...
		fHM->Create3<TH3D>("fhRingTrackDistDiffXVsXYTruematch"+s, "fhRingTrackDistDiffXVsXYTruematch"+s+";X [cm];Y [cm];Y Ring-track distance [cm]", nBinsX, xMin, xMax, nBinsY, yMin, yMax, 100, 0., 5.);
		fHM->Create3<TH3D>("fhRingTrackDistDiffYVsXYTruematch"+s, "fhRingTrackDistDiffYVsXYTruematch"+s+";X [cm];Y [cm];X Ring-track distance [cm]", nBinsX, xMin, xMax, nBinsY, yMin, yMax, 100, 0., 5.);
    }
    ...
    for(Int_t iTrack = 0; iTrack < nofGlobalTracks; iTrack++) {
        ...
        for (int i = 0; i < 2; i++){
			...
			if (stsMcTrackId == richMcTrackId) {
				...
				fHM->H3("fhRingTrackDistDiffXVsXYTruematch"+s)->Fill(xc, yc, rtDistanceX);
				fHM->H3("fhRingTrackDistDiffYVsXYTruematch"+s)->Fill(xc, yc, rtDistanceY);
...
void CbmRichRecoQa::DrawRingTrackDistHistWithSuffix(const string& s )
{
    ...
    {
    	TCanvas* c = fHM->CreateCanvas("fh_ring_track_distance_difference_x_y_vs_xy_truematch"+s, "fh_ring_track_distance_difference_x_y_vs_xy_truematch"+s, 1800, 600);
    	c->Divide(2, 1);
    	c->cd(1);
    	DrawH3Profile(fHM->H3("fhRingTrackDistDiffXVsXYTruematch"+s), true, false, 0., 2.);
    	c->cd(2);
    	DrawH3Profile(fHM->H3("fhRingTrackDistDiffYVsXYTruematch"+s), true, false, 0., 2.);
    }
}
