...
Bool_t CbmRichPmt::isPhotonDetected(CbmRichPmtTypeEnum detType, Double_t momentum)
{
    ...
    if (it == fPmtDataMap.end()){
        LOG(ERROR) << "CbmRichPmt::isPhotonDetected - Wrong detector type :" << detType << FairLogger::endl;;
        return false;
    }

    CbmRichPmtQEData* pmtData = it->second;
    if (NULL == pmtData) {
        LOG(ERROR) << "CbmRichPmt::isPhotonDetected - data is NULL for detType :" << detType << FairLogger::endl;
        return false;
    }
    ...
    if (lambda >= pmtData->fLambdaMin && lambda < pmtData->fLambdaMax) {
       ...
       if (ilambda < 0 || ilambda >= pmtData->fEfficiency.size()) {
...
TGraph* CbmRichPmt::getQEGraph(CbmRichPmtTypeEnum detType)
{
    ...
    CbmRichPmtQEData* pmtData = it->second;
    if (NULL == pmtData) {
        cout << "CbmRichPmt::getQEGraph - data is NULL for detType :" << detType << endl;
        return NULL;
    }
