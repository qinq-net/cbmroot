 ...
 * Modified 18/04/2017 by Alex Bercuci <abercuci@niham.nipne.ro>
 *
 *                                 3         2         1          shift length
 ...
 * Row ID on bits 17-22           00000000011111100000000000000000 <<17 63
 * Column ID on bits 23-30        01111111100000000000000000000000 <<23 255
 * Triangle up/down on bits 31    10000000000000000000000000000000 <<31 1
 *
 **/
...
public:
   ...
   * \param[in] triangId Triangle ID.
   * \return Address from system ID, layer, module, sector, row, column and triangle IDs.
   **/
   static UInt_t GetAddress(Int_t layerId,
      ...
      assert(!(layerId < 0 || layerId > fgkLayerIdLength || moduleId < 0 || moduleId > fgkModuleIdLength
            ...
            || columnId < 0 || columnId > fgkColumnIdLength || triangId < 0 || triangId > fgkTriangleIdLength));
      return (kTrd << fgkSystemIdShift) | (layerId << fgkLayerIdShift) | (moduleId << fgkModuleIdShift) |
            (sectorId << fgkSectorIdShift) | (rowId << fgkRowIdShift) | (columnId << fgkColumnIdShift) | (triangId << fgkTriangleIdShift);
    }
   ...
   **/
   ...
   /**
   * \brief Return triangle ID from address.
   * \param[in] address Unique channel address.
   * \return Triangle ID from address.
   **/
   static UInt_t GetTriangleId(UInt_t address) {
      return (address & (fgkTriangleIdLength << fgkTriangleIdShift)) >> fgkTriangleIdShift;
   }
   ...
   **/
   ...
   static UInt_t SetLayerId(UInt_t address, Int_t newLayerId) {
      ...
      return GetAddress(newLayerId, GetModuleId(address), GetSectorId(address), GetRowId(address), GetColumnId(address), GetTriangleId(address));
   }
   ...
   static UInt_t SetModuleId(UInt_t address, Int_t newModuleId) {
      ...
      return GetAddress(GetLayerId(address), newModuleId, GetSectorId(address), GetRowId(address), GetColumnId(address), GetTriangleId(address));
   }
   ...
   static UInt_t SetSectorId(UInt_t address, Int_t newSectorId) {
      ...
      return GetAddress(GetLayerId(address), GetModuleId(address), newSectorId, GetRowId(address), GetColumnId(address), GetTriangleId(address));
   }
   ...
   static UInt_t SetRowId(UInt_t address, Int_t newRowId) {
      ...
      return GetAddress(GetLayerId(address), GetModuleId(address), GetSectorId(address), newRowId, GetColumnId(address), GetTriangleId(address));
   }
   ...
   static UInt_t SetColumnId(UInt_t address, Int_t newColumnId) {
      ...
      return GetAddress(GetLayerId(address), GetModuleId(address), GetSectorId(address), GetRowId(address), newColumnId, GetTriangleId(address));
   }

   /**
    * \brief Set new triangle ID for address.
    * \param address Current address.
    * \param newTriangId New value for triangle ID.
    * \return New address with modified triangle ID.
    */
   static UInt_t SetTriangleId(UInt_t address, Int_t newTriangId) {
      assert(!(newTriangId < 0 || newTriangId > fgkTriangleIdLength));
      return GetAddress(GetLayerId(address), GetModuleId(address), GetSectorId(address), GetRowId(address), GetColumnId(address), newTriangId);
   }
...
private:
   ...
   static const Int_t fgkRowIdLength         =  63; // 2^6 - 1
   static const Int_t fgkColumnIdLength      = 255; // 2^8 - 1
   static const Int_t fgkTriangleIdLength    =   1; // 2^1 - 1
   ...
   static const Int_t fgkColumnIdShift       = 23;
   static const Int_t fgkTriangleIdShift     = 31;
