...
{
 public:
  L1EventEfficiencies():names(),indices(),ratio_reco(),ratio_ghosts(0),mc(),reco(),ghosts(0),nEvents(0),
                        ratio_clone(),
                        clone()
  {
  ...
  void CalcEff(){
    ratio_reco = reco/mc;
    const double total = reco.counters[indices["reconstructable"]] + ghosts + clone.counters[indices["reconstructable"]];
    if (total > 0)
