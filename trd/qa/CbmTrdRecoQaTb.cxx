...
{
  printf("Exec() :: digits[%x] clusters[%x] hits[%x]\n", (void*)fTrdDigis, (void*)fTrdClusters, (void*)fTrdHits);
//  printf("Exec() :: digits[%d] clusters[%d] hits[%d]\n", fTrdDigis->GetEntriesFast(), fTrdClusters->GetEntriesFast(), fTrdHits->GetEntriesFast());
  string type;
  if ( fDaq ) {
    type = "TimeSlice";
    cout<<fTimeSlice->ToString()<<endl;
    cout<<fTimeSlice->GetMatch().ToString()<<endl;
...
void CbmTrdRecoQaTb::ReadDataBranches()
{
  ...
  if ( fDaq ) {
    ...
    if ( NULL == fTrdDigis )
      LOG(ERROR) << GetName() << ": No TrdDigi array!" << FairLogger::endl;
  }

  fTrdClusters = (TClonesArray*) ioman->GetObject("TrdCluster");
  if ( NULL == fTrdClusters )
    LOG(ERROR) << GetName() << ": No TrdCluster array!" << FairLogger::endl;

  fTrdHits = (TClonesArray*) ioman->GetObject("TrdHit");
  if ( NULL == fTrdHits )
    LOG(ERROR) << GetName() << ": No TrdHit array!" << FairLogger::endl;

//   fTrdClusterMatches = (TClonesArray*) ioman->GetObject("TrdClusterMatch");
//   if ( NULL == fTrdClusterMatches )
//     LOG(ERROR) << GetName() << ": No TrdClusterMatch array!" << FairLogger::endl;
// 
//   fTrdHitMatches = (TClonesArray*) ioman->GetObject("TrdHitMatch");
//   if ( NULL == fTrdHitMatches )
//     LOG(ERROR) << GetName() << ": No TrdHitMatch array!" << FairLogger::endl;
}
...
{
  printf("ProcessDigisAndPoints(TClonesArray*) : digis[%x] type[%s]\n", (void*)digis, type.c_str());
  ...
  for(Int_t iDigi = 0; iDigi < digis->GetEntriesFast(); iDigi++) {
    const CbmTrdDigi* digi = static_cast<const CbmTrdDigi*>(digis->At(iDigi));
    cout<<iDigi<<" : "<<digi->ToString()<<endl; continue;
    const CbmMatch* digiMatch = static_cast<const CbmMatch*>(digi->GetMatch());
    Int_t stationId = 0;//fSetup->GetStationNumber(stsDigi->GetAddress());
    for(Int_t iLink = 0; iLink < digiMatch->GetNofLinks(); iLink++) {
      const CbmLink link = digiMatch->GetLink(iLink);
      Double_t index = (1000 * link.GetIndex()) + (link.GetFile()) + (0.0001 * link.GetEntry());
