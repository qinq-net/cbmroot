...
CbmTrdModule::CbmTrdModule(
        ...
        const TArrayD& sectorSizeX, const TArrayD& sectorSizeY,
        const TArrayD& padSizeX, const TArrayD& padSizeY,
        const Bool_t padGeoTriangular, const Bool_t fasp)
  : TNamed(),                                 
...
void CbmTrdModule::InitAsics() {
  Int_t iAsic = 0;
  Int_t nChannels = 32;
  Int_t iFebGroup = fTriangularPads?(fAsicFASP?2:1):0; // 1; 2;  // normal, super, ultimate
...
//___________________________________________________________________________
void  CbmTrdModule::Print(Option_t *opt) const
{
/** 
  Dump formated parameters for this module
*/
  printf(" CbmTrdModule @ %5d rotation[%2ddeg] pads[%s] rows[%2d] cols[%2d]\n"
         "        center   [%7.2f %7.2f %7.2f]\n"
         "        size     [%7.2f %7.2f %7.2f]\n"
         "        anode    pitch[%4.2f] d2PP[%4.2f] off[%4.2f]\n"
         "        pads    ",
         fModuleAddress, fOrientation*90, fTriangularPads?"triangular":"rectangular",
         GetNofRows(), GetNofColumns(),
         fX, fY, fZ, fSizeX, fSizeY, fSizeZ,
         fAnodeWireSpacing, fAnodeWireToPadPlaneDistance, fAnodeWireOffset);
  for(Int_t isec(0); isec<fNofSectors; isec++) printf(" [c(%3d) r(%2d)]x[w(%4.2f) h(%5.2f)]", GetNofColumnsInSector(isec), GetNofRowsInSector(isec), fPadSizeX.At(isec), fPadSizeY.At(isec)); printf("\n");
  printf("        asics    %s[%d]    \n", (fAsicFASP?"FASP":"SPADIC"), fNofAsics);
  if(strcmp(opt, "asic")!=0) return;
  
  for(Int_t iasc(0); iasc<fNofAsics; iasc++) ((const CbmTrdAsic*)fAsicMap.at(iasc))->Print("addr"); 
  printf("\n");
}
...
//___________________________________________________________________________
Int_t CbmTrdModule::GetSectorRow(Int_t growId, Int_t& rowId) const
{
  // check limits
  if ( (growId < 0) || (growId >= GetNofRows()) )
    LOG(FATAL) << "CbmTrdModule::GetSectorRow rowId "   << growId     << " of " << GetNofRows()
    << " in moduleAddress " << fModuleAddress << " is out of bounds!" << FairLogger::endl;
