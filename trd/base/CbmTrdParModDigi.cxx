...
*/  
  ...
  if (fAnodeWireOffset > 0.0 && fAnodeWireSpacing > 0.0) {  // wires must be defined defined
    ...
    LOG(ERROR) << "CbmTrdParModDigi::ProjectPositionToNextAnodeWire: fAnodeWireOffset and fAnodeWireSpacing not set. ProjectPositionToNextAnodeWire can not be used." << FairLogger::endl;
  }
}
...
*/
  ...
  LOG(ERROR) << "CbmTrdParModDigi::GetSector: Could not find local point in any of the sectors" << FairLogger::endl;
...
*/
  ...
  if ( (npady < 0) || (npady > GetNofRows()-1) ) {
    LOG(ERROR) << "CbmTrdParModDigi::GetSector - there is no such row number: " << npady << FairLogger::endl;
    return -1;
  }
      ...
      if ( npady < nofRows + secRows ) {  // if we are in the sector containing the pad
        ...
        if ( (rowId < 0) || (rowId > GetNofRowsInSector(iSector)-1) )
          LOG(FATAL) << "CbmTrdParModDigi::GetModuleRow rowId "   << rowId     << " of " << GetNofRowsInSector(iSector)-1
                      ...
                      << " is out of bounds!" << FairLogger::endl;
  ...
  LOG(ERROR) << "CbmTrdParModDigi::GetSector: Could not find pad in any of the sectors" << FairLogger::endl;
  return -1; // This should be never reached
}
...
//     LOG(ERROR) << "CbmTrdParModDigi::GetModuleInformation: This is wrong!" << FairLogger::endl;
//   }
 ...
 */
  ...
  // check limits
  if ( fabs(posX) > fSizeX )
    LOG(FATAL) << "CbmTrdParModDigi::GetPadPosition posX=" << posX << " is out of bounds!" << FairLogger::endl;
  // check limits
  if ( fabs(posY) > fSizeY )
    LOG(FATAL) << "CbmTrdParModDigi::GetPadPosition posY=" << posY << " is out of bounds!" << FairLogger::endl;
 ...
 */
  ...
  //  std::cout<< "  sector: "<< sectorId<<"   row: " << rowId<<"   col: " << columnId<<"   size x: " << padsizex<<"  size y: "<< padsizey<<" posx: "<< posX<<" posY: "<< posY<<std::endl;
...
*/
//   if (fModuleAddress != moduleAddress) {
//     LOG(ERROR) << "CbmTrdParModDigi::GetPosition This is wrong!" << FairLogger::endl;
//   }
