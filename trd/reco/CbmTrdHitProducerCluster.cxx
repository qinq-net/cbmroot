...
void CbmTrdHitProducerCluster::TriangularPadReconstructionAB(Int_t clusterId)
{
  ...
  Double_t totalCharge[2] = {0.};
  Double_t totalChargeTR[2] = {0.};
  Int_t nofDigis = cluster->GetNofDigis();

  Int_t digiId = -1;
  Int_t combiId(-1), secId(-1), colId(-1), rowId(-1), rowOld(-1), nCol(0);
...
//       padH = moduleInfo->GetPadSizeY(secId);
    ...
    Double_t  chargeR(digi->GetCharge(1)+TMath::Max(0.,digi->GetChargeTR(1))),
              chargeT(digi->GetCharge(0)+TMath::Max(0.,digi->GetChargeTR(0)));
    //printf("  AB:: r[%2i] c[%2i] t[%d] q[%7.4f / %7.4f]\n", rowId, colId, Int_t(digi->GetTime()), chargeR, chargeT);
    // pack digits to T/R coupling for further processing
    combiId = setTriangleCombId(rowId, colId, nCol, 0);
    digiMap[combiId].first= chargeR; digiMap[combiId].second= UChar_t(digi->GetTime());    
    digiMap[combiId+1].first= chargeT; digiMap[combiId+1].second  = UChar_t(digi->GetTime());    
    totalCharge[0]  += digi->GetCharge(0);totalCharge[1]  += digi->GetCharge(1);
    totalChargeTR[0]+= digi->GetChargeTR(0);totalChargeTR[1]+= digi->GetChargeTR(1);
    rowOld          =  rowId;
  }  
...
{
  //printf("  TriangularProcessRowCluster [%d] module[%x]...\n", fRecoTriangular, (void*)fModule);
  ...
  // TODO should be stored in digi param for each channel according to calibration
  const Int_t sThresholdADC = 800;
  //fModule->Print();
  ...
  for(std::map<UInt_t, std::pair<Double_t, UChar_t>>::iterator id=digiMap.begin(); id!=digiMap.end(); ++id){ 
    Int_t sADC=Int_t(id->second.first)/*-ECalib[0])/ECalib[1]*/;
    //getTriangleCombId(id->first, fModule->GetNofColumns(), r, c, t); printf("\t%d[%c] r[%2d] c[%2d] s[keV]=%7.2f s[ADC]=%4d t[%d]\n", id->first, (t?'T':'R'), r, c, id->second.first, sADC, id->second.second);
    //if(sADC<sThresholdADC) continue;
    // convert signal[GeV] to ADC based on calibration
  ...
  while(kTRUE){
    ...
    if(maxS<sThresholdADC) break;
    //printf("  MAX mid[%d] ly[%d] s[%d] r[%2d | %2d] c[%2d] q[%7.4f]\n", mid, ly, maxSec, maxRow, r, maxCol, maxS);
    ...
    //printf("====================\n");  
    // look to the right
    lastS = maxS; lastT=maxT; 
    if(mp!=padMap.end()){
      mp++;
      for(ip=mp; ip!=padMap.end(); ip++){
        //printf("  rPad[%4d] : mask[%c] q[%f] time[%d]\n", ip->first, (ip->second.stat&1)?'y':'n', ip->second.signal, ip->second.time);
        if(ip->second.stat&1) break;// detect other cluster
        Double_t sgn(ip->second.signal); UChar_t tt(ip->second.time);
        if( (sgn<MAXS && lastS<MAXS) && // detect hit stop only for signals which are not in saturation
          ((sgn>lastS) ||                         // detect increase in signal
    ...
    for(ip=lp; 1; ip++){
      ...
      if(ip==mp) imax=idx; 
//       printf(" *");} else printf("  ");
//       printf("pad[%4d] : mask[%c] q_%d[%7.2f] time[%d]\n", ip->first, (ip->second.stat&1)?'y':'n', idx, ip->second.signal, ip->second.time);
      idx++;
      if(ip==rp) break;
    }  
    ...
    TVector3 loc_xyz; 
    loc_xyz.SetXYZ(padPos[0]+2*x*padSize[0],                      // offset from column center 
 ...
 */
  ...
  //if(max%2) jpad++; // take care of the case when the maximum is on a tilted pad
  ...
  //if(an==-5) fTriangularSignal->Print();
