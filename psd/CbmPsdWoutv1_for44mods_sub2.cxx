...
#include "CbmPsdWoutv1_for44mods_sub2.h"
...
// -----   Default constructor   -------------------------------------------
CbmPsdWoutv1_for44mods_sub2::CbmPsdWoutv1_for44mods_sub2() 
: CbmPsd(),
  fTrackID(0),
  fVolumeID(0),
  fModuleID(0),
...
CbmPsdWoutv1_for44mods_sub2::CbmPsdWoutv1_for44mods_sub2(const char* name, Bool_t active)
: CbmPsd(name, active),
  fTrackID(0),
  fVolumeID(0),
  fModuleID(0),
...
Bool_t  CbmPsdWoutv1_for44mods_sub2::ProcessHits(FairVolume* /*vol*/)
{
  //  cout<<" CbmPsdWoutv1_for44mods_sub2::ProcessHits in "<<vol->GetName()<<endl;
...
//    Int_t iCell, iNumm ;
  ...
  //if(gMC->VolId("VHSS") == gMC->CurrentVolID(copyNoVHLE)) return kFALSE;
...
void CbmPsdWoutv1_for44mods_sub2::CreateMaterial() {
}
// -----   Public method ConstructGeometry   -------------------------------
void CbmPsdWoutv1_for44mods_sub2::ConstructGeometry() {
...
//gGeoManager->MakeBox("WOUT",med11, 70, 70, 0.2);
  gGeoManager->MakeBox("VWOT",med2, 40, 40, 0.05);
  TGeoVolume *WOUT = gGeoManager->MakeBox("WOUT",med2, 40, 40, 0.05);
  gGeoManager->MakeBox("VHLL",med1,10,10,62.4);//hole
//gGeoManager->MakeBox("VHLE",med1,10,10,0.2);//hole
//gGeoManager->MakeTube("VHLT",med1,0,3,0.2);//hole
  gGeoManager->MakeBox("VHLE",med2,10,10,0.05);//hole
  gGeoManager->MakeBox("VHSS",med2,20,20,0.05);//hole
...
/*
  ...
  TGeoVolume *VWOT = gGeoManager->MakeBox("VWOT",med2, 40, 40, 0.05);
  TGeoVolume *WOUT = gGeoManager->MakeBox("WOUT",med2, 40, 40, 0.05);
  ...
  TGeoVolume *VHSS = gGeoManager->MakeBox("VHSS",med2,20,20,0.05);//hole
...
//Holes in WOUT
  gGeoManager->Node("VHSS", 1, "WOUT", 0., 0., 0., 0,kTRUE, buf, 0); 

  /*
  //gGeoManager->Node("VHLE", 1, "WOUT", 0., 0., 0., 0,kTRUE, buf, 0);  gGeoManager->Node("VHLT", 1, "WOUT", 0., 0., 0., 0,kTRUE, buf, 0); //comment for w/o hole
  gGeoManager->Node("VHLE", 1, "WOUT", 70., 50., 0., 0,kTRUE, buf, 0); 
  gGeoManager->Node("VHLE", 2, "WOUT", -70., 50., 0., 0,kTRUE, buf, 0); 
  gGeoManager->Node("VHLE", 3, "WOUT", 70., -50., 0., 0,kTRUE, buf, 0); 
  gGeoManager->Node("VHLE", 4, "WOUT", -70., -50., 0., 0,kTRUE, buf, 0); 
  */
  /*
...
ClassImp(CbmPsdWoutv1_for44mods_sub2)
