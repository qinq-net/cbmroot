...
#include "CbmPsdWoutv1_for44mods_sub3.h"
...
// -----   Default constructor   -------------------------------------------
CbmPsdWoutv1_for44mods_sub3::CbmPsdWoutv1_for44mods_sub3() 
: CbmPsd(),
  fTrackID(0),
  fVolumeID(0),
  fModuleID(0),
...
CbmPsdWoutv1_for44mods_sub3::CbmPsdWoutv1_for44mods_sub3(const char* name, Bool_t active)
: CbmPsd(name, active), 
  fTrackID(0),
  fVolumeID(0),
  fModuleID(0),
...
Bool_t  CbmPsdWoutv1_for44mods_sub3::ProcessHits(FairVolume* /*vol*/)
{
  //  cout<<" CbmPsdWoutv1_for44mods_sub3::ProcessHits in "<<vol->GetName()<<endl;
...
//    Int_t iCell, iNumm ;
  ...
  //if(gMC->VolId("VHLS") == gMC->CurrentVolID(copyNoVHLE)) return kFALSE;
  ...
  if ( gMC->IsTrackEntering() ) {
    ...
    */
    ...
    if(fPos.X()>-18.6 && fPos.X()<61.4 && TMath::Abs(fPos.Y())<20) cout <<"MARINA " <<fPos.X() <<" " <<fPos.Y() <<" " <<fPos.Z() <<" " <<pdg <<" " <<gMC->GetStack()->GetCurrentParentTrackNumber() <<" " <<gMC->CurrentVolName() <<endl;
...
void CbmPsdWoutv1_for44mods_sub3::CreateMaterial() {
}
// -----   Public method ConstructGeometry   -------------------------------
void CbmPsdWoutv1_for44mods_sub3::ConstructGeometry() {
...
//  TGeoMedium   *medium   = 0;
  ...
  gGeoManager->MakeBox("VWOT",med2, 80, 60, 0.05);
  TGeoVolume *WOUT = gGeoManager->MakeBox("WOUT",med2, 80, 60, 0.05);
  ...
  gGeoManager->MakeBox("VHLS",med2,40,40,0.05);//hole
...
/*
  ...
  //TGeoVolume *WOUT = gGeoManager->MakeBox("WOUT",med11, 70, 70, 0.2);
  TGeoVolume *VWOT = gGeoManager->MakeBox("VWOT",med2, 80, 60, 0.05);
  TGeoVolume *WOUT = gGeoManager->MakeBox("WOUT",med2, 80, 60, 0.05);
  ...
  TGeoVolume *VHLS = gGeoManager->MakeBox("VHLS",med2,40,40,0.05);//hole
...
//Holes in WOUT
  ...
  gGeoManager->Node("VHLS", 1, "WOUT", 0., 0., 0., 0,kTRUE, buf, 0); //comment for w/o hole
...
ClassImp(CbmPsdWoutv1_for44mods_sub3)
