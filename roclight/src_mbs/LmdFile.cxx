...
extern "C" {
  #include "sMbs.h"
  #define FILEONLY // <- Anyway just hack to read old LMD data files
  #include "fLmd.h"
}
...
bool mbs::LmdFile::OpenRead(const char* fname, uint32_t buffersize)
{
   ...
   printf("mbs::LmdFile::OpenRead => Buffer size: %u filename: %s\n", buffersize, fname);
...
//   sMbsHeader* hdr = fLmdGetElement((sLmdControl*) fControl, LMD__NO_INDEX);
   sMbsHeader* hdr = NULL;
   fLmdGetElement((sLmdControl*) fControl, LMD__NO_INDEX, &hdr);
