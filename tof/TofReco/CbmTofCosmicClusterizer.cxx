...
void CbmTofCosmicClusterizer::Exec(Option_t* /*option*/)
{
   ...
   LOG(DEBUG)<<"CbmTofCosmicClusterizer::Exec => New event"<<FairLogger::endl;
   fStart.Set();
...
Bool_t   CbmTofCosmicClusterizer::RegisterInputs()
{
   ...
   if( NULL == fManager)
   {
      LOG(ERROR)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not find FairRootManager!!!"<<FairLogger::endl;
      return kFALSE;
   ...
   {
      LOG(ERROR)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not get the TofPoint TClonesArray!!!"<<FairLogger::endl;
      return kFALSE;
   } // if( NULL == fTofPointsColl)
   ...
   {
      LOG(ERROR)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not get the MCTrack TClonesArray!!!"<<FairLogger::endl;
      return kFALSE;
   } // if( NULL == fMcTracksColl)
   */
   ...
   if( NULL == fTofDigisColl)
   {
      LOG(ERROR)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not get the CbmTofDigi TClonesArray!!!"<<FairLogger::endl;
      return kFALSE;
   } // if( NULL == fTofDigisColl)
   ...
   if( NULL == fTrbHeader)
   {
      LOG(INFO)<<"CbmTofCosmicClusterizer::RegisterInputs => Could not get the TofTrbHeader Object"<<FairLogger::endl;
   }
...
Bool_t   CbmTofCosmicClusterizer::InitParameters()
{
   ...
   if( 0 == fDigiPar )
   {
      LOG(ERROR)<<"CbmTofCosmicClusterizer::InitParameters => Could not obtain the CbmTofDigiPar "<<FairLogger::endl;
      return kFALSE; 
   }
   ...
   if( 0 == fDigiBdfPar )
   {
      LOG(ERROR)<<"CbmTofCosmicClusterizer::InitParameters => Could not obtain the CbmTofDigiBdfPar "<<FairLogger::endl;
      return kFALSE; 
   }
...
Bool_t   CbmTofCosmicClusterizer::FillHistos()
{
 ...
 // plot remaining time difference to previous hits 
   ...
   for (Int_t iSel=0; iSel<iNSel; iSel++){
	 ...
	 if(fDutRpc>-1) {iR0=fDutRpc; iRl=fviClusterMul[fDutId][fDutSm].size();}

	 for(Int_t iRpc=iR0; iRpc<iRl; iRpc++) iDutMul += fviClusterMul[fDutId][fDutSm][iRpc];
          LOG(DEBUG)<<"CbmTofCosmicClusterizer::FillHistos(): selector 0: DutMul "
		    <<fviClusterMul[fDutId][fDutSm][fDutRpc]<<", "<<iDutMul
		    <<", Sel2Mul "<<iSel2Mul<<" TRef: "<<dTRef
		    <<FairLogger::endl;
         if(   iDutMul>0  && iDutMul < fiCluMulMax ) {                           
             LOG(DEBUG1)<<"CbmTofCosmicClusterizer::FillHistos(): Found selector 0"<<FairLogger::endl;
	 ...
	 for(Int_t iRpc=iR0; iRpc<iRl; iRpc++) iRefMul += fviClusterMul[fSelId][fSelSm][iRpc];
          LOG(DEBUG)<<"CbmTofCosmicClusterizer::FillHistos(): selector 1: RefMul "
		    <<fviClusterMul[fSelId][fSelSm][fSelRpc]<<", "<<iRefMul
		    <<", Sel2Mul "<<iSel2Mul<<FairLogger::endl;
         if(   iRefMul>0  && iRefMul < fiCluMulMax  ) {                        
             LOG(DEBUG1)<<"CbmTofCosmicClusterizer::FillHistos(): Found selector 1"<<FairLogger::endl;
             dTTrig[iSel]=dDoubleMax;
             for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
             {
              pHit = (CbmTofHit*) fTofHitsColl->At( iHitInd );
       ...
       default :
       LOG(INFO)<<"CbmTofCosmicClusterizer::FillHistos: selection not implemented "<<iSel<<FairLogger::endl;
         ;
     }  // switch end 
     if(fTRefMode>10){
 ...
 for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
 {
   ...
   if(TMath::Abs(hitpos_local[1])>fChannelInfo->GetSizey()*fPosYMaxScal) continue;
	 ...
	 // Fill event selected histograms 
	   ...
	   if(fvLastHits[iSmType][iSm][iRpc][iCh].size()>1){     // check for previous hits in memory time interval 
	     ...
	     dTTcor[iSel] = dDelTof;   // store timing correction
	     dZsign[iSel]=1.;	
	 ...
	 if(pHit->GetZ() < pRef->GetZ())  dDelTof *= -1.;   // store timing correction
...
Bool_t CbmTofCosmicClusterizer::AddNextChan(Int_t iSmType, Int_t iSm, Int_t iRpc, Int_t iLastChan, Double_t dLastPosX, Double_t dLastPosY, Double_t dLastTime, Double_t dLastTotS){
  ...
  if( 1 < fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size() )
  {
    ...
    for(Int_t i1=0; i1<fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size()-1; i1++)
    {
      ...
      {
	LOG(DEBUG)<<"check digi pair "<<i1<<","<<i2<<" with size "<<fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size()<<FairLogger::endl;
	...
	if(TMath::Abs(dTime-dLastTime)<fdMaxTimeDist){
	  ...
	  Double_t dPosY=0.;
	    ...
	    if (it != fStorDigiInd[iSmType][iSm*iNbRpc+iRpc][iCh].end()){
	      ...
	      LOG(DEBUG)<<"Found i2 "<<i2<<" with Ind2 "<<Ind2<<" at position "<< ipos <<FairLogger::endl; 
	    ...
	    if ( iCh != (iNbCh-1) && AddNextChan(iSmType,iSm,iRpc,iCh,dLastPosX,dLastPosY,dLastTime,dLastTotS) ){
	      LOG(DEBUG)<<"Added Strip "<<iCh<<" to cluster of size "<<dNClHits<<FairLogger::endl;
