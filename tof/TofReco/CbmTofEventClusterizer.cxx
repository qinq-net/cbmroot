...
{
  if(fTofCalDigisCollOut)  fTofCalDigisCollOut->Delete();
  if(fEventsColl) {
    LOG(DEBUG)<<"CbmTofEventClusterizer::Exec => New time slice with "
	      << fEventsColl->GetEntriesFast() << " events, "
	      << fTofRawDigisColl->GetEntriesFast() << " TOF digis " 
	      <<FairLogger::endl;
    ...
    for(Int_t iEvent = 0; iEvent < fEventsColl->GetEntriesFast(); iEvent++)
    {
      ...
      for (Int_t iDigi = 0; iDigi < tEvent->GetNofData(kTofDigi); iDigi++)
      {
        ...
        CbmTofDigiExp* tDigi = dynamic_cast<CbmTofDigiExp*>(fTofRawDigisColl->At(iDigiIndex));
	new((*fTofDigisColl)[iNbDigis++]) CbmTofDigiExp(*tDigi); 
      }
      ...
      for (Int_t index = 0; index < fTofCalDigisColl->GetEntriesFast(); index++){
        CbmTofDigiExp* tDigi = dynamic_cast<CbmTofDigiExp*>(fTofCalDigisColl->At(index));
	tEvent->AddData(kTofCalDigi, iNbCalDigis);
	new((*fTofCalDigisCollOut)[iNbCalDigis++]) CbmTofDigiExp(*tDigi); 
      }
	...
	tEvent->AddData(kTofHit, iNbHits);
	...
	*/
	...
	iNbHits++;
      ...
      fTofCalDigisColl->Delete();//Clear("C"); //otherwise memoryleak! FIXME
...
void CbmTofEventClusterizer::ExecEvent(Option_t* /*option*/)
{
   ...
   LOG(DEBUG)<<"CbmTofEventClusterizer::Exec => New event"<<FairLogger::endl;
   fStart.Set();
...
Bool_t   CbmTofEventClusterizer::RegisterInputs()
{
   ...
   if( NULL == fManager)
   {
      LOG(ERROR)<<"CbmTofEventClusterizer::RegisterInputs => Could not find FairRootManager!!!"<<FairLogger::endl;
      return kFALSE;
   ...
   {
      LOG(ERROR)<<"CbmTofEventClusterizer::RegisterInputs => Could not get the TofPoint TClonesArray!!!"<<FairLogger::endl;
      return kFALSE;
   } // if( NULL == fTofPointsColl)
   ...
   {
      LOG(ERROR)<<"CbmTofEventClusterizer::RegisterInputs => Could not get the MCTrack TClonesArray!!!"<<FairLogger::endl;
      return kFALSE;
   } // if( NULL == fMcTracksColl)
   */
   fEventsColl = dynamic_cast<TClonesArray*>(fManager->GetObject("Event"));
     ...
     fTofRawDigisColl   = (TClonesArray *) fManager->GetObject("CbmTofDigiExp");

     if( NULL == fTofRawDigisColl)
       fTofRawDigisColl = (TClonesArray *) fManager->GetObject("CbmTofDigi");

     if( NULL == fTofRawDigisColl)
       fTofRawDigisColl = (TClonesArray *) fManager->GetObject("TofDigiExp");

     if( NULL == fTofRawDigisColl)
       fTofRawDigisColl = (TClonesArray *) fManager->GetObject("TofDigi");
  
     if( NULL == fTofRawDigisColl) {
       LOG(ERROR)<<"CbmTofEventClusterizer::RegisterInputs => Could not get the CbmTofDigi TClonesArray!!!"<<FairLogger::endl;
       return kFALSE;
     } // if( NULL == fTofDigisColl)
     ...
     if( NULL == fTrbHeader)   {
       LOG(INFO)<<"CbmTofEventClusterizer::RegisterInputs => Could not get TofTrbHeader Object"<<FairLogger::endl;
     }
   if (NULL == fEventsColl) {
   } else {
     // time based input 
     LOG(INFO) << "CbmEvent found in input file, assume time based input" << FairLogger::endl;
...
Bool_t   CbmTofEventClusterizer::InitParameters()
{
   ...
   if( 0 == fDigiPar )
   {
      LOG(ERROR)<<"CbmTofEventClusterizer::InitParameters => Could not obtain the CbmTofDigiPar "<<FairLogger::endl;
      return kFALSE; 
   }
   ...
   if( 0 == fDigiBdfPar )
   {
      LOG(ERROR)<<"CbmTofEventClusterizer::InitParameters => Could not obtain the CbmTofDigiBdfPar "<<FairLogger::endl;
      return kFALSE; 
   }
  ...
  */
  ...
  // read parameter from histos
    ...
    */
	...
	LOG(INFO)<<"Svel histogram not found for module type "<<iSmType<<FairLogger::endl;
      }
   ...
   for (Int_t iS=0; iS< fDigiBdfPar->GetNbSmTypes(); iS++){
     ...
     */
     ...
     for (Int_t iPar =0; iPar<4; iPar++) {
       ...
       if(NULL == hFparcur){
	 LOG(INFO)<<"Histo "<< Form("cl_SmT%01d_Fpar%1d", iS, iPar) <<" not found, recreate ..."<<FairLogger::endl;
   ...
   for (Int_t iSel=0; iSel<iNSel; iSel++){
	 ...
	 for(Int_t iRpc=iR0; iRpc<iRl; iRpc++) iDutMul += fviClusterMul[fDutId][fDutSm][iRpc];
         ...
         if(   iDutMul>0  && iDutMul < fiCluMulMax ) {                           
             ...
             LOG(DEBUG1)<<"Found selector 0, NbHits  "<<iNbTofHits<<FairLogger::endl;
             for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
             {
              pHit = (CbmTofHit*) fTofHitsColl->At( iHitInd );
              if(NULL == pHit) continue;
	 ...
	 for(Int_t iRpc=iR0; iRpc<iRl; iRpc++) iRefMul += fviClusterMul[fSelId][fSelSm][iRpc];
         ...
         if(   iRefMul>0  && iRefMul < fiCluMulMax  ) {                        
             LOG(DEBUG1)<<"CbmTofEventClusterizer::FillHistos(): Found selector 1"<<FairLogger::endl;
             dTTrig[iSel]=dDoubleMax;
             for( Int_t iHitInd = 0; iHitInd < iNbTofHits; iHitInd++)
             {
              pHit = (CbmTofHit*) fTofHitsColl->At( iHitInd );
       ...
       default :
       LOG(INFO)<<"CbmTofEventClusterizer::FillHistos: selection not implemented "<<iSel<<FairLogger::endl;
         ;
     }  // switch end 
     if(fTRefMode>10){
       dTTrig[iSel]=dTRef;
	   ...
	   {  if (pDigi->GetChannel() == pDigi2->GetChannel())
		...
		if(dTDif<dTDifMin){
		 ...
		 for (; iDigI3<iNbTofDigi;iDigI3++){
		   CbmTofDigiExp *pDigi3 = (CbmTofDigiExp*) fTofDigisColl->At( iDigI3 );
		 ...
		 if(iDigI3 == iNbTofDigi) // same side neighbour did not fire 
		 {
		   ...
		   case 2:  // insert missing hits
		     CbmTofDigiExp *pDigiN  = new((*fTofDigisColl)[iNbTofDigi++]) CbmTofDigiExp( *pDigi );
		     pDigiN->SetAddress(pDigi->GetSm(),pDigi->GetRpc(),pDigi2->GetChannel(),pDigi->GetSide(),pDigi->GetType());
		     pDigiN->SetTot(pDigi2->GetTot());

		     CbmTofDigiExp *pDigiN2 = new((*fTofDigisColl)[iNbTofDigi++]) CbmTofDigiExp( *pDigi2 );
...
Bool_t CbmTofEventClusterizer::AddNextChan(Int_t iSmType, Int_t iSm, Int_t iRpc, Int_t iLastChan, Double_t dLastPosX, Double_t dLastPosY, Double_t dLastTime, Double_t dLastTotS){
  ...
  while( fvDeadStrips[iDetIndx] & ( 1 << iCh )) {
    LOG(DEBUG) << "Skip channel "<<iCh<<" of detector "<<Form("0x%08x",iDetId)<<FairLogger::endl;
    iCh++; iLastChan++;
  ...
  if( 1 < fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size() )
  {
    ...
    for(Int_t i1=0; i1<fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size()-1; i1++)
    {
      ...
      {
	LOG(DEBUG1)<<"check digi pair "<<i1<<","<<i2<<" with size "<<fStorDigiExp[iSmType][iSm*iNbRpc+iRpc][iCh].size()<<FairLogger::endl;
	...
	if(TMath::Abs(dTime-dLastTime)<fdMaxTimeDist){
	  ...
	  Double_t dPosY=0.;
	    ...
	    if (it != fStorDigiInd[iSmType][iSm*iNbRpc+iRpc][iCh].end()){
	      ...
	      LOG(DEBUG1)<<"Found i2 "<<i2<<" with Ind2 "<<Ind2<<" at position "<< ipos <<FairLogger::endl; 
	      ...
	      LOG(FATAL)<<" Did not find  i2 "<<i2<<" with Ind2 "<<Ind2<<FairLogger::endl; 
	    }
	    ...
	    if ( iCh != (iNbCh-1) && AddNextChan(iSmType,iSm,iRpc,iCh,dLastPosX,dLastPosY,dLastTime,dLastTotS) ){
	      LOG(DEBUG1)<<"Added Strip "<<iCh<<" to cluster of size "<<dNClHits<<FairLogger::endl;
...
Bool_t   CbmTofEventClusterizer::CalibRawDigis()
{
  ...
  for( Int_t iDigInd = 0; iDigInd < iNbTofDigi; iDigInd++ ) {
	...
	LOG(INFO)<<"CbmTofEventClusterizer::BuildClusters: CalDigi "
		 <<iDigIndCal<<",  T "
		 <<pCalDigi->GetType()<<", Sm "
		 <<pCalDigi->GetSm()<<", R "
		 <<pCalDigi->GetRpc()<<", Ch "
...
void CbmTofEventClusterizer::SetDeadStrips (Int_t iDet, Int_t ival){ 
  if( fvDeadStrips.size() < iDet+1 ) fvDeadStrips.resize(iDet+1);
