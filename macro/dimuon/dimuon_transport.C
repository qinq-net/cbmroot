...
void dimuon_transport(Int_t nEvents = 2,
                   ...
                   const char* inputFile = "")
{
  ...
  TString inputSignal = srcDir + "/macro/much/data/jpsi.root";
  std::cout << "-I- " << myName << ": Using background input file " << inFile << std::endl;
  std::cout << "-I- " << myName << ": Using signal input file " << inputSignal << std::endl;
  // ------------------------------------------------------------------------
  ...
  // Use the CbmUnigenGenrator for the background
  CbmUnigenGenerator*  uniGen = new CbmUnigenGenerator(inFile);
  primGen->AddGenerator(uniGen);
  // Use the CbmPlutoGenerator for the signal
