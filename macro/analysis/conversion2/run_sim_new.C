...
void run_sim_new(Int_t nEvents = 2, Int_t file_nr = 1, const char* setup = "light_sis100_electron")
{
...
//  TString outDir  = "data/";
  ...
  TString outFile = outDir + setup + "_test.mc."+ temp +".root";
  TString parFile = outDir + setup + "_params."+ temp +".root";
  TString geoFile = outDir + setup + "_geofile_full."+ temp +".root";
  ...
  TString plutoFile = "/common/cbma/users/kres/software/pluto_v5.43/macros/Ievgenii_pluto_pi0_no_dalitz_10GeV."+ temp + ".root";
  TString inFile  = "/common/cbmb/Users/kres/simulations/inputfiles/urqmd/urqmd.auau.10gev.centr."+ temp + ".root";

  // Function needed for CTest runtime dependency
//  TString depFile = Remove_CTest_Dependency_File(outDir, "run_sim_new" , setup);
...
/*        FairBoxGenerator* boxGen1 = new FairBoxGenerator(11, 1);
        ...
        FairBoxGenerator* boxGen2 = new FairBoxGenerator(-11, 2);
...
/*	FairBoxGenerator* boxGen4 = new FairBoxGenerator(111, 50);
	boxGen4->SetPtRange(0.,6.);
	boxGen4->SetPhiRange(0.,360.);
	//boxGen4->SetThetaRange(0.,45.);		// set theta range (not at the same time as SetYRange() )
	boxGen4->SetYRange(0., 6.);				// set Y range (not at the same time as SetThetaRange() )
	//boxGen4->SetCosTheta();				// set distribution of theta values (standard = flat)
	boxGen4->Init();
	primGen->AddGenerator(boxGen4);*/


//	FairAsciiGenerator* asciiGen = new FairAsciiGenerator("data/generator/my_first_generator.dat");
