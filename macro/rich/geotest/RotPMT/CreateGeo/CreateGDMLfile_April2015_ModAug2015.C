#include <iostream>
#include <fstream>  
void CreateGDMLfile_April2015_ModAug2015(float pmt_pos_y_addend = 0, float pmt_pos_z_addend = 0, int RotMir=-10, float PMTrotX=5, float PMTrotY=5 , int DefaultDims=0, int DefaultDims_LargePMT=0){
  ...
  if(DefaultDims==1){
    ...
    if(DefaultDims_LargePMT==1){Delta_pmt_width=400.; Delta_pmt_height=300.;}
  ...
  //Path1="/hera/cbm/users/tariq/cbmroot";}
  sprintf(GeoFileName,"/geometry/rich/GeoOpt/rich_geo_%s_RotPMT_%s_%s_TransPMT_%s_%s%s",RotMirText,ShiftXTxt,ShiftYTxt,YTransText,ZTransText, ExtraText.Data());
