...
FLAG=1		# select geometry	# (== 0) for aligned case ; (== 1) for misaligned 5 mrad case ; (== 2) for misaligned 1 mrad case ; (== 3) for standard v16a_1e geo
...
##	sbatch -J position -D /lustre/nyx/cbm/users/jbendar/Sim_Outputs/Ring_Track_VS_Position/Misaligned_5mrad_Full --time=6:00:00 --array=1-200 ./position.sh $FLAG $NEVTS $COLL_ENERGY
##	sbatch -J position -D /lustre/nyx/cbm/users/jbendar/Sim_Outputs/Ring_Track_VS_Position/Misaligned_1mrad_Full --time=4:00:00 --array=1-200 ./position.sh $FLAG $NEVTS $COLL_ENERGY
	sbatch -J position -D /lustre/nyx/cbm/users/jbendar/Sim_Outputs/test/test_position/ --time=2:00:00 --array=1-200 ./position.sh $FLAG $NEVTS $COLL_ENERGY
# done
