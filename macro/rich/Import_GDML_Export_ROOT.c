void Import_GDML_Export_ROOT()
{
	...
	TGeoVolume* gdmlTop = parser.GDMLReadFile (vmcDir + "/geometry/rich/rich_v18a_1e.gdml");
	...
	TFile* outfile = new TFile(vmcDir + "/geometry/rich/rich_v18a_1e.geo.root", "RECREATE");
	rootTop->Write();
	outfile->Close();
}
