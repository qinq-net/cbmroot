...
////DE   new TGeoTrd2("stsCone", r1, r2, r1, r2, stsZ/2.);
  ...
  for (Int_t iPlate = 0; iPlate < 9; iPlate++) {
    ...
    TGeoShape* innerPlate = new TGeoBBox(Form("innerPlate%02d",iPlate), maxDx*2. + 6.2 + 1e-7, maxDy, plateSizes[iPlate][2] + 1e-7);
...
//    Double_t posZ = statPos[iUnit/2] - stsPosZ;
  ...
  for (Int_t iPlate = 0; iPlate < 9; iPlate++) {
    ...
    infoFile << Form("%.1f x %.1f x %.1f cm, Positioned %.1f cm downstream the target", 2. * plateSizes[iPlate][0], 2. * plateSizes[iPlate][1], 2. * plateSizes[iPlate][2], stsPosZ + plateZ) << endl;
  }
  ...
  for (Int_t i=0; i < sensorNodes->GetEntriesFast(); i++)
  {
    ...
    if (projectedRadius < (distanceToCenter - sensorFittingRadius))
      continue;
    ...
    TGeoBBox *newShape = new TGeoBBox(newName, oldShape->GetDX(), oldShape->GetDY(), oldShape->GetDZ() + gkAddedSensorThickness / 2.);
    TGeoVolume *vol = gGeoManager->MakeBox(newName, gGeoMan->GetMedium("silicon"), oldShape->GetDX(), oldShape->GetDY(), oldShape->GetDZ() + gkAddedSensorThickness / 2.);
