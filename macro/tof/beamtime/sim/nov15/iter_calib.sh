...
IterIndices=( $(find ${CalibDir} -maxdepth 1 -type d -name 'Iter*' -printf '%f\n' | cut -c5- | sort -n) )

for iIterIndex in ${IterIndices[@]}
do
  if [ "$iIterIndex" -gt "$iRestart" ]; then
    rm -rf ${CalibDir}/Iter${iIterIndex}
  fi
done


#for iMode in 1 6 8
for iMode in 0 12 13 0
do
  ...
  case $iMode in 
    ...
    2)
      ...
      OptList=(
        '1'${c0}'0,92,1,'${iMRef}',1'
        '1'${c0}'00,64,1,-'${iMRef}',1'
        '1'${c0}'00,64,1,'${iBRef}',1'
        '1'${c0}'0,92,0,'${iDut}',1'
        '1'${c0}'00,64,0,-'${iDut}',1'
        '1'${c0}'00,64,0,'${iBRef}',1'
        '1'${c0}'0,92,1,'${iMRef}',1'
      )
      ;;
    3)
  ...
  while [ "${iNIterations}" -gt "0" ]; do
    ...
    do
      ...
      if [ "$iStep" -gt "$iRestart" ]; then
        ...
        root -b -q ''${MacroDir}'/ana_digi.C('$inOpt',"'${UnpackDir}'",'${iDut}','${iMRef}','${iBRef}','${iSel2}','${iDeadTime}',0)'
