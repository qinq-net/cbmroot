...
const Bool_t IncludePowerbusbars= true;  // false;
...
void create_power_bars()
{
  ...
  {
    power_1->AddNode(trdbox1, 4, tr203);
    power_1->AddNode(trdbox1, 7, tr206);
  }

  Int_t (l=0; l<MaxLayers DFDFDF
  for DEDEDE
  if (ShowLayer[0])  // if geometry contains layer 1 (1st layer of station 1)
    gGeoMan->GetVolume(geoVersion)->AddNode(power_1, 1, tz1);
  if (ShowLayer[0])  // if geometry contains layer 1 (1st layer of station 1)
    gGeoMan->GetVolume(geoVersion)->AddNode(power_1, 1, tz1);
  if (ShowLayer[0])  // if geometry contains layer 1 (1st layer of station 1)
    gGeoMan->GetVolume(geoVersion)->AddNode(power_1, 1, tz1);
