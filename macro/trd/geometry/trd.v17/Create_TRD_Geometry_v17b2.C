...
const Double_t feb_rotation_angle[NofModuleTypes] = { 45,  60,  90,  80,  80,  90,  90,  90 }; // number of GBTx ROBs on module
...
//        (*incline_feb) =  (*trd_feb_trans1) * (*trd_feb_rotation) * (*trd_feb_trans2) * (*trd_feb_y_position);  // not OK 
	...
	TGeoRotation    *trd_asic_rotation;   // rotation around x axis
        ...
        trd_asic_trans1     = new TGeoTranslation("", 0., -(feb_thickness + asic_offset + asic_thickness/2.), -febvol_thickness/2.);  // move ASIC center to FEB corner
	trd_asic_trans2     = new TGeoTranslation("", 0.,   feb_thickness + asic_offset + asic_thickness/2. ,  febvol_thickness/2.);  // move FEB corner back to asic center
