...
void create_rich_v18b_mcbm()
{
	...
	TGeoVolume *caveVol = gGeoMan->MakeBox("rich_v18b_mcbm", medAl, (boxWidth + boxThickness)/2. , (boxHeight + boxThickness)/2., (boxLength + boxThickness)/2.);
	...
	//Positioning
	topVol->AddNode(caveVol, 1, trCave[0]);
	topVol->AddNode(caveVol, 1, trCave[1]);
	topVol->AddNode(caveVol, 3, trCave[2]);
	topVol->AddNode(caveVol, 4, trCave[3]);
