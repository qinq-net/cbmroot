...
const Int_t   LayerType[MaxLayers]        = {20, 10, 11, 10, 11, 20, 21, 20, 21, 30};  // ab: a [1-4] - layer type, b [0,1] - vertical/horizontal pads
...
//          Int_t copy = copy_nr(stationNr, copyNrOut[type - 5], isRotated, PlaneId[layerId], modId);
	  ...
	  //          layer_angle    = atan( (DetectorSizeX[1]/2. - FrameWidth[1]) / (zfront[setupid] + layerId * LayerThickness) );
          layer_angle    = atan( (DetectorSizeX[1]/2. - FrameWidth[1]) / (zfront[setupid] + 15 + (layerId - 1) * LayerThickness) );
	  ...
	  //        xPos = 0;
          ...
          layer_angle    = atan( (DetectorSizeX[1]/2. - FrameWidth[1] + xPos) / (zfront[setupid] + 15 + (layerId - 1) * LayerThickness) );
