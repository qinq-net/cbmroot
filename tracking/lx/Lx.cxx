...
bool LxHitFile::WriteHit(Int_t stationNumber, Int_t layerNumber, Double_t x, Double_t y, Double_t z, Double_t xErr, Double_t yErr, Double_t zErr)
{
  if (evPtr + 2 * sizeof(Int_t) + 6 * sizeof(Double_t) > evEnd)
    return false;
...
bool LxHitFile::ReadEvent(Int_t eventNumber)
{
  ...
  if (eventNumber >= numFooterEntries)
    return false;
...
bool LxHitFile::ReadHit(Int_t& stationNumber, Int_t& layerNumber, Double_t& x, Double_t& y, Double_t& z, Double_t& xErr, Double_t& yErr, Double_t& zErr)
{
  if (evPtr + 2 * sizeof(Int_t) + 6 * sizeof(Double_t) > evEnd)
    return false;
...
#pragma omp critical
  ...
  signalInterTracksAngle = new TH1F("signalInterTracksAngle", "Angle between signal tracks by hits on 1st station", 200, 0.0, 60.0);
  signalInterTracksAngle->StatOverflows();
  bgrInterTracksAngle = new TH1F("bgrInterTracksAngle", "Angle between background tracks by hits on 1st station", 200, 0.0, 60.0);
  bgrInterTracksAngle->StatOverflows();
  signalInterTrackCorrDA = new TH2F("signalInterTrackCorrDA", "signalInterTrackCorrDA", 200, 0.0, 250.0, 200, 0.0, 60.0);
...
#ifdef MAKE_TRIGGERING_HISTOS
  triggeringAllTracksVertices = new TH1F("triggeringAllTracksVertices", "triggeringAllTracksVertices", 330, -10.0, 320.0);
  triggeringAllTracksVertices->StatOverflows();
  triggeringDistTracksVertices = new TH1F("triggeringDistTracksVertices", "triggeringDistTracksVertices", 330, -10.0, 320.0);
  triggeringDistTracksVertices->StatOverflows();
  triggeringSignTracksVertices = new TH1F("triggeringSignTracksVertices", "triggeringSignTracksVertices", 330, -10.0, 320.0);
  triggeringSignTracksVertices->StatOverflows();
  triggeringTrigTracksVertices = new TH1F("triggeringTrigTracksVertices", "triggeringTrigTracksVertices", 330, -10.0, 320.0);
...
void LxFinder::Exec(Option_t* opt)
{
...

  {
    ...
    cout << "Signal true hypotheses: " << result << "% ( " << trueSignalTriggerings << " / " << signalCounter << " )" << endl;
