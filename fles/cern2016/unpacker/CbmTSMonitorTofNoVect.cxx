...
Bool_t CbmTSMonitorTofNoVect::DoUnpack(const fles::Timeslice& ts, size_t component)
{
     ...
     if( fUnpackPar->IsChannelRateEnabled() )
     {
       ...
       if( fTsLastHit.end() != fTsLastHit.find( rocId ) )
       {
         ...
         if( fTsLastHit[rocId].end() != fTsLastHit[rocId].find( get4Id ) )
         {
           ...
           {
             ChannelRate_gDPB[gdpbNr]->Fill( 1e9/ ( mess.getMsgFullTimeD( curEpochGdpbGet4 )
       ...
       fTsLastHit[rocId][get4Id][channel] = mess.getMsgFullTimeD( curEpochGdpbGet4 );
     } // if( fUnpackPar->IsChannelRateEnabled() )
